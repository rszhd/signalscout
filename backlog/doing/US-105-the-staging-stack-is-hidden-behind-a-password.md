...
- [ ] The app's own login and session still work behind the prompt, because
      Traefik strips the `Authorization` header before the app reads it
...
- Traefik v3 `basicauth` with `removeHeader` defaulting to true is what keeps
  the app's login working. Confirm this on the version the proxy pins
  (`traefik:v3.7`, `docker-compose.proxy.yml:28`).
