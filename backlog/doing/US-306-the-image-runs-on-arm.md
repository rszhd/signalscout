...
- [ ] `docker manifest inspect` on `latest` and on a version tag lists both.
      **Not yet true**: it is true of `latest` after the next push to `main`
      and of a version after the next release. Check it then.
...
- [ ] The Log records what the change did to the CI build time. **Waiting
      on the first run**; the local build is recorded below but says nothing
      about a runner.
