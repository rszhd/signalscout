...
**This ticket is `packages/engine`: 106 blocks over 15 lines, outside tests.**
The connectors and the model calls. `sources/types.ts` is 76% comment, and the four
provider clients carry a paragraph each about what the wire format did on the
day it was read.
...
**Four files are US-258's**, not this ticket's: `packages/engine/src/sources/types.ts` and
`packages/engine/src/sources/providers/apify/linkedin.ts` belong to US-258. Do US-258
...
- [ ] `node scripts/comment-density.mjs --blocks --min=15` is run first, and
      every block it names under `packages/engine` outside tests is read.
