...
- [ ] `AI_POLICY.md` exists at the root and says, in this order: AI-written
...
- [ ] It says plainly that the owner writes with an agent, that the owner is
      the one human who has read the code, and what "reviewed" means here
      (the Acceptance list, the tests, and what ran live).
- [ ] CONTRIBUTING.md links it from *Before you write code*, and the DCO
      section says `-s` is not optional.
- [ ] The pull request template (US-294) has an *AI use* line: none /
      assisted / generated, plus the tool.
- [ ] `.githooks/pre-commit`, or a `commit-msg` hook beside it, refuses a
...
- [ ] The owner's own agent configuration writes `Assisted-by:` and signs
      off from this ticket onward. The Log records the first commit that
      does.
- [ ] Existing history is not rewritten. The policy says the old trailer is
