...
stylesheets still apart. `monitor-setup.css`: 53 the same, 16 different, 32 only hosted, 3 only here. The form's steps differ by product (five and a budget here, one guided flow hosted); its fields, cards, step header and actions do not.
...
- `apps/web/src/styles/monitor-setup.css`, `MonitorForm.tsx`, and the same in `signalscout-cloud`.
